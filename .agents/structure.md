...
vuecs/
  packages/           # Component + infrastructure packages (npm workspaces)
    ...
    overlays/         # @vuecs/overlays — Modal (+ useModal view-stack), AlertDialog (Reka AlertDialog confirm primitive + useConfirm() / VCConfirmDialog host, plan 040), Popover, HoverCard (plan 013), Tooltip (+ TooltipProvider), DropdownMenu, ContextMenu, Toast (+ useToast shared queue, plan 029) — all on Reka primitives
