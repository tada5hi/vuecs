...
vuecs's design-token + runtime palette architecture is **directly inspired by [Nuxt UI](https://github.com/nuxt/ui)**. The semantic-color naming, `setColorPalette()`-style runtime switcher, dark-mode-via-CSS-variables approach, and the structural-class-vs-theme-class split all map to ideas Nuxt UI shipped first. The key difference: Nuxt UI is Nuxt-only and ties itself to `tailwind-variants` + Tailwind v4 explicitly, while vuecs's theme system is **framework- and CSS-library-agnostic** — you can stack Bootstrap v4/v5 themes alongside the Tailwind one.
...
| **Configured at install** | `nuxt.config.ts` → `ui.theme.colors: ['primary', 'success', …]` | `app.use(vuecs, { themes: [...] })` + `setColorPalette({ primary: 'green' })` |
| **Color → palette binding** | `app.config.ts` → `ui.colors.primary: 'green'` (build-time + runtime via `useAppConfig`) | `--vc-color-primary-* → var(--color-green-*)` rewrite via `<style id="vc-color-palette">` (runtime, no rebuild) |
...
| **Color config** | `app.config.ts` `ui.colors` (build-time hint + runtime) | `themes/tailwind/src/palette.ts` `setColorPalette()` — pure runtime (post-017; the generic primitives `applyColorPaletteCss` / `bindColorPalette<T>` live in `packages/design/src/palette.ts`) |
| **Composables** | `useUI()`, `useAppConfig()`, `useColorMode()` | `useComponentTheme()`, `useComponentDefaults()`, `useColorPalette()`, `useColorMode()` |
...
- **Runtime palette as `<style>` block.** `setColorPalette()` writes a plain CSS `<style id="vc-color-palette">` instead of mutating an `appConfig` ref. Works in any framework, not just Nuxt.
