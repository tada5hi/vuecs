...
Bulma 1.0+ theme for vuecs components. Ships an optional `--bulma-*` → `--vc-color-*` bridge so `setColorPalette()` re-tints both vuecs and Bulma-native components in real time.
