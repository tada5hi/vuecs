...
Compound list components for Vue 3: a stateful root that provides context and thin part components (`Header`, `Body`, `Item`, `Footer`, `Loading`, `NoMore`) that read from it. Plus a `useList()` composable for shared state across views. Successor to `@vuecs/list-controls` — clean break, compound API.
