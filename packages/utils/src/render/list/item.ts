...
export function buildListItemOptions<T extends Record<string, any>>(
    ...
    return {
        ...
        iconProps: unrefWithDefault(options.iconProps, {
            class: 'fa fa-bars pr-1',
        }),

        textPropName: unrefWithDefault(options.textPropName, 'name'),
...
export function buildListItem<T extends Record<string, any>>(
    ...
    if (options.icon) {
        // class: pr-1
        children.push(h('div', [h('i', options.iconProps)]));
    }
    ...
    return h(options.type, { key: options.index, ...options.props }, children);
}
