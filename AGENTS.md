...
| `@vuecs/link` | Router-aware link component (vue-router/nuxt). Public prop type is `LinkProps`, derived from the prop declaration via `ExtractPublicPropTypes` — the hand-written `LinkProperties` mirror (stale, and index-signature'd so typos compiled) was removed in #1705. | 1.0.1 |
...
| `@vuecs/overlays` | Compound overlays on Reka primitives — Modal (+ `useModal()` view-stack composable), AlertDialog (Reka `AlertDialog`; the `role="alertdialog"` confirm primitive — `Trigger`/`Content`/`Title`/`Description`/`Cancel`/`Action`) + the imperative `useAlertDialog()` → `Promise<boolean>` composable + single `<VCAlertDialogProvider>` host (plan 040), Popover, HoverCard, Tooltip, DropdownMenu, ContextMenu, Toast (+ `useToast()` shared queue composable, plan 029) | 0.0.0 |
