...
| `@vuecs/overlays` | Compound overlays on Reka primitives — Modal (+ `useModal()` view-stack composable), AlertDialog (Reka `AlertDialog`; the `role="alertdialog"` confirm primitive — `Trigger`/`Content`/`Title`/`Description`/`Cancel`/`Action`) + the imperative `useConfirm()` → `Promise<boolean>` composable + single `<VCConfirmDialog>` host (plan 040), Popover, HoverCard, Tooltip, DropdownMenu, ContextMenu, Toast (+ `useToast()` shared queue composable, plan 029) | 0.0.0 |
