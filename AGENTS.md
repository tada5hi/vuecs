...
| `@vuecs/table` | Compound table (`VCTable` + `VCTableLite` + Header / Body / Footer / Row / Cell / HeadCell / Empty / Loading) with a `:columns :data` driver, default cell renderer (`accessor` + `formatter`), driver auto-render of header/body, stacked responsive mode, controlled `v-model:sort` on `VCTable`, opt-in row-click + row keyboard nav, `_rowVariant` / `_cellVariants` row-meta. Layer 1 — `@vuecs/core` peer dep only; semantic HTML, no Reka primitive. (plans 028 + 033) | 0.0.0 |
