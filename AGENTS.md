...
| `@vuecs/navigation` | Multi-level navigation — per-call-site `:resolver` items + a shared reactive registry for dependent navs (plan 037, 3.0 clean break, no NavigationManager) + `<VCStepper>` compound for multi-step wizards | 2.4.1 |
