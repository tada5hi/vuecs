...
| `@vuecs/table` | Compound table (`VCTable` + `VCTableLite` + Header / Body / Footer / Row / Cell / HeadCell / Empty / Loading / SortIndicators / Placeholder / ExpandTrigger / RowExpansion) with a `:columns :data` driver, default cell renderer (`accessor` + `formatter`), driver auto-render of header/body, stacked responsive mode, controlled `v-model:sort` on `VCTable` with multi-column + client-side sort, opt-in row-click + row keyboard nav, `_rowVariant` / `_cellVariants` row-meta, a discoverable `<VCTableSortIndicators>` chip-row for managing multi-sort without Shift-click, `<VCTablePlaceholder>` skeleton sibling for loading states, and per-row expansion panels (`<VCTable :expandable>` + `#expansion` slot — ResizeObserver-measured height animation, W3C disclosure ARIA). Layer 1 — depends on `@vuecs/core` + `reka-ui` (the latter only for `<Presence>`-backed expansion unmount delay). (plans 028 + 033 + 038, issue #1476) | 0.0.0 |
...
`navigation` also depends on `@vuecs/link`. `pagination`, `overlays`, `forms`, `elements`, `navigation`, and `table` take a runtime dep on `reka-ui` (they wrap Reka's headless primitives — pagination, dialog/popover/hover-card/tooltip/menu, the form-input families, the atomic separator/avatar/aspect-ratio/visually-hidden elements, the stepper, and `<Presence>`-driven expansion-row unmount delay respectively). `theme-tailwind` is designed to pair with `@vuecs/design` (Tailwind v4 + CSS-variable tokens); `theme-bootstrap` and `theme-bulma` each ship an optional bridge mapping their framework's runtime CSS variables (`--bs-*`, `--bulma-*`) onto the design-system tokens. Nuxt consumers install `@vuecs/nuxt` regardless of theme; the module's palette / color-mode SSR plugins dispatch through whatever themes the user installs (plan 025 collapsed the previous per-theme Nuxt sub-module split).
